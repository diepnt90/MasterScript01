...
function collect_counters()
{
    ...
    local counters_file="${WORK_DIR}/${instance}_${label}_${timestamp}_counters.csv"

    echo "$(date '+%Y-%m-%d %H:%M:%S'): [${label}] Collecting dotnet-counters (10s, csv) -> $(basename $counters_file) ..." | tee -a "$output_file"
